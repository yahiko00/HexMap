...
module Heightmap {
  export var rng: SeededRNG;

  export function init(rng: SeededRNG = new SeededRNG()) {
    this.rng = rng;
  } // init
   ...
   */
  export function diamondSquare(MAP_SIZE: number, low: number = 0, high: number = 255, wrap: boolean = true, roughness: number = 1): number[][] {
    ...
    map[0][0] = low + rng.rand() * (high - low); // top left
    map[0][MAP_SIZE - 1] = low + rng.rand() * (high - low); // bottom left
    map[MAP_SIZE - 1][0] = low + rng.rand() * (high - low); // top right
    map[MAP_SIZE - 1][MAP_SIZE - 1] = low + rng.rand() * (high - low); // bottom right
  ...
  function offset(height, roughness) {
    ...
    // in the range (-h, +h)
    return (rng.rand() * 2 - 1) * height * roughness;
  } // offset
